...
- **start** (`SetGroupData`): velocity = the object's own object.dat break velocity (`CObjectInfo+0x3C`, e.g. lamp posts `0,0,0.1` m/step) plus random ± its break-velocity scatter (`+0x48`). Each piece spins at 3–6° per step about a random axis. Every joint of the broken mesh's physics asset is broken (`FindConstraintBoneName` for each constraint, then `BreakConstraint`), so the pieces move independently, like the original groups. The log line `original pieces: N groups, M joints broken` shows how many.
- **flight** (`Update`): gravity `timeStep/125` per step (2 g, as in the original). After 5 steps the piece stops tumbling and turns its flat face to the ground normal at `angle·timeStep/20`. As in `CalcGroupCenter`, the flat face is the one across the piece's thinnest axis, measured on its collision at the break, so an upright pole piece lies down instead of staying upright.
